...
現状の `index.html` では、ページ読み込みから参加者が実験を開始するまでに以下の順序でデータが取得されます。

1. `DOMContentLoaded` 直後に `loadFromLocalStorage()` を呼び出し、Supabase に接続できない場合でも最低限の設定・キャッシュを復元します。
2. `initializeSupabase()` で Supabase クライアントを初期化し、正常に接続できたら `loadDataFromSupabase()` を実行します。ここでは管理設定・シナリオ・同意書設定・アンケート定義のみを読み込み、**実験ログ／同意履歴／サポートメッセージなどの全件取得は行いません**。
3. 管理者パスワードで認証したとき、または管理タブ（ログ、同意履歴、サポートメッセージ、アンケート結果）を開いたときに、初めて対応するテーブルを全件取得します。これにより参加者モードでの初期負荷を避けています。
4. 参加者が ID を入力して実験を開始するときは、開始直前にその参加者 ID の実験ログだけを取得し、`initializeUserSession` が自分の `user_id` だけを `user_sessions` テーブルから読み込みます。`getOrCreateAssignment` も自分の割り当て (`participant_assignments`) のみを参照・作成する挙動になっており、記録保存時も `participant_id` を条件に絞っています。

これらの処理は `DOMContentLoaded` 内の初期化ブロックや各ユーティリティ関数にまとまっているため、どの段階で全件ロードが走るかを把握しやすくなっています。重さが気になる場合は、`loadDataFromSupabase()` 内の一括取得を必要なテーブルだけに分割する、といった対策を検討してください。

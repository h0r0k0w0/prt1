...
export default async function handler(req, res) {
  ...
  const requestPayload = {
    ...
    presence_penalty: toNumber(body.presence_penalty ?? 0.1, 0.1),
    frequency_penalty: toNumber(body.frequency_penalty ?? 0.1, 0.1),
  };

  if (body.max_completion_tokens != null) {
    requestPayload.max_completion_tokens = toNumber(body.max_completion_tokens, 300);
  } else {
    requestPayload.max_tokens = toNumber(body.maxTokens ?? body.max_tokens, 300);
  }

  try {
    console.log(`Using model: ${requestPayload.model}, max tokens: ${requestPayload.max_tokens ?? requestPayload.max_completion_tokens}, temperature: ${requestPayload.temperature}`);
